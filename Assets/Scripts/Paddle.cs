...
public class Paddle : MonoBehaviour {
	...
	void AutoPlay(){
		...
		this.transform.position = new Vector3(Mathf.Clamp(ballPos.x,-4.0f,4.0f),-7.0f,0);
	
	}
	#endregion
	
	#region MoveWithMouse()
	void MoveWithMouse(){
		/*
		...
		ROZWIAZANIE Z KURSU*/
		mousePosInBlocks = Input.mousePosition.x / Screen.width * 1.5f ;
		this.transform.position = new Vector3(Mathf.Clamp(mousePosInBlocks,-5.5f,5.5f),-8.0f,0f);
	}
	#endregion
}
